...
import (
	...
	"github.com/open-telemetry/opentelemetry-collector-contrib/processor/jsprocessor/internal/metadata"
)
...
func createMetricsProcessor(ctx context.Context, set processor.Settings, cfg component.Config, nextConsumer consumer.Metrics) (processor.Metrics, error) {
	return processorhelper.NewMetrics(ctx, set, cfg, nextConsumer, newJSProcessor().processMetrics, processorhelper.WithCapabilities(processorCapabilities))
}
...
func createTracesProcessor(ctx context.Context, set processor.Settings, cfg component.Config, nextConsumer consumer.Traces) (processor.Traces, error) {
	return processorhelper.NewTraces(ctx, set, cfg, nextConsumer, newJSProcessor().processTraces, processorhelper.WithCapabilities(processorCapabilities))
}
...
func createLogsProcessor(ctx context.Context, set processor.Settings, cfg component.Config, nextConsumer consumer.Logs) (processor.Logs, error) {
	return processorhelper.NewLogs(ctx, set, cfg, nextConsumer, newJSProcessor().processLogs, processorhelper.WithCapabilities(processorCapabilities))
}
